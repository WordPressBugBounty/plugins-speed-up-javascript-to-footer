	...
	public function preload_scripts() {
		...
		foreach ( $wp_scripts->queue as $handle ) {
			...
			if ( empty( $wp_scripts->registered[ $handle ]->src ) ) {
				// Gli handle "alias" servono solo a raggruppare dipendenze.
				continue;
			}

			// Gli script registrati con un commento condizionale sono per
			// Internet Explorer, che nessun browser in circolazione e' piu':
			// precaricarli sarebbe banda buttata via.
			if ( isset( $wp_scripts->registered[ $handle ]->extra['conditional'] ) ) {
				continue;
			}

			// src arriva da cio' che tema e plugin hanno registrato: va escapata
			// prima di finire nell'HTML.
			echo '<link rel="preload" href="' . esc_url( $wp_scripts->registered[ $handle ]->src ) . '" as="script">' . "\r\n";
		}
	}
}
